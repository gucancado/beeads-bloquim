      ...
      <div className="flex-1 overflow-auto bg-slate-50 dark:bg-background">
        <div className="max-w-6xl mx-auto p-8 lg:p-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-12">
            <div>
              <h1 className="text-4xl font-display font-bold text-foreground">Workspaces</h1>
              <p className="text-muted-foreground mt-2 text-lg">Manage your teams and projects</p>
            </div>
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              ...
              <DialogContent className="sm:max-w-md rounded-2xl">
                <DialogHeader>
                  <DialogTitle className="text-2xl font-display">Create Workspace</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleCreate} className="space-y-6 mt-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Workspace Name</label>
                    <Input 
                      placeholder="e.g. Engineering Team, Personal Projects" 
                      value={name}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24 bg-card rounded-3xl border border-border shadow-sm">
              ...
              <h3 className="text-2xl font-bold font-display text-foreground">No workspaces yet</h3>
              <p className="text-muted-foreground mt-2 mb-8 max-w-md mx-auto">Create a workspace to start organizing your mind maps and tasks.</p>
                ...
                <Link key={ws.id} href={`/workspaces/${ws.id}`}>
                  <div className="group bg-card p-6 rounded-3xl border border-border/60 shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-300 cursor-pointer flex flex-col h-full hover:-translate-y-1">
                    ...
                    <div className="mt-auto pt-6 flex items-center justify-between text-sm text-muted-foreground">
                      <span>{ws.role.charAt(0).toUpperCase() + ws.role.slice(1)}</span>
